...
$rows = [];
try {
  $st = $pdo->prepare("SELECT id, name, email, role, created_at FROM users WHERE role='admin' ORDER BY id DESC");
  $st->execute();
  $rows = $st->fetchAll();
} catch (Throwable $e) {
  ...
  <main class="content">
    <div style="display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap;align-items:end;">
      <div>
        ...
        <div style="color:#666;margin-top:6px;">Daftar user role <b>admin</b>.</div>
      </div>
        ...
        <table style="width:100%;border-collapse:collapse;min-width:760px;">
          <thead>
            <tr>
              ...
              <th style="text-align:left;border-bottom:1px solid #eee;padding:10px;">Dibuat</th>
              <th style="text-align:left;border-bottom:1px solid #eee;padding:10px;">Aksi</th>
            </tr>
          </thead>
          <tbody>
            <?php if (!$rows): ?>
              <tr><td colspan="5" style="padding:12px;color:#666;">Belum ada admin.</td></tr>
            <?php else: ?>
                ...
                <tr>
                  ...
                  <td style="padding:10px;border-bottom:1px solid #f3f3f3;display:flex;gap:8px;flex-wrap:wrap;">
                    ...
                    <form method="post" action="<?= e(url('admin/users/delete.php')) ?>" style="margin:0;">
                      ...
                      <input type="hidden" name="back" value="index_admin.php">
                      <button class="btn" type="submit" onclick="return confirm('Hapus admin ini?')">Hapus</button>
                    </form>
                  </td>
                </tr>
              <?php endforeach; ?>
